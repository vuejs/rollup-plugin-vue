...
export function createVuePartRequest(filename: string, lang: string | undefined, type: string, index?: number): string {
  ...
  query[PARAM_NAME] = [type, index, lang].filter(it => it !== undefined).join('.')
